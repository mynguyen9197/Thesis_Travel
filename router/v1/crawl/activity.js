  ...
  try {
    ...
    await page.goto(url + '#photos;aggregationId=&albumid=101&filter=7').then(async() => {
      page.content().then(async(content) =>{
        ...
        const name = $('.ui_header').text()
        const foundName = await activity.loadByName(name)
        if(foundName.length !== 0) return
        let rating = $('.ui_header').next().find('span').attr('class').split('_')
        rating = parseFloat(rating[rating.length - 1])/10
        const review = $('.ui_header').next().find('span').eq(1).text().split(' ')[0]
        ...
        const ranking = '', kind_of_place = []
        const overview = $('.ui_header').parent().parent().text()
        console.log({name, rating, review, ranking, kind_of_place, overview})
